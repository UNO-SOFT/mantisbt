...
if( $t_box_title === 'vv' || $t_box_title === 'owned' ) {
	...
	if( $t_box_title === 'vv' ) {
		...
		$t_url_link_parameters['vv'] = [
			FILTER_PROPERTY_SEARCH => 'Verzióváltó', 
			FILTER_PROPERTY_REPORTER_ID => $t_developers,
		];
		$t_bug_ids = filter_verziovaltok(); 
	} elseif( $t_box_title == 'owned' && $t_owned_field_id ) {
		$t_url_link_parameters['owned'] = [
			FILTER_PROPERTY_HIDE_STATUS => $t_bug_resolved_status_threshold,
			'custom_field_' . $t_owned_field_id => $t_current_user_name,
		];
...
<tr class="my-buglist-bug <?php echo $t_bug_class?>">
	...
	<td class="<?php echo $t_status_label?> nowrap width-13 my-buglist-id">
		<?php
